...
all_true <- rep(TRUE, nrow(rentals))
all_false <- rep(FALSE, nrow(rentals))
false_true <- all_true
false_true[1:(0.1 * nrow(rentals))] <- FALSE
scatterplot(rentals, selected = false_true)
scatterplot(rentals, selected = all_true)
overlay_histogram(rentals, selected = false_true)
```
...
ui <- fluidPage(
  ...
  fluidRow(
    column(6,
      plotOutput("histogram", brush = brushOpts("plot_brush", direction = "x"), height = 600),
...
server <- function(input, output) {
  ...
  observeEvent(
    input$plot_brush,
    {
        tmp <- brushedPoints(rentals, input$plot_brush, allRows = TRUE)
        selected(tmp$selected_)
        #selected(# what lies in the brush)
    }
  )
  
  output$map <- renderPlot({
      scatterplot(rentals, selected())
  })
