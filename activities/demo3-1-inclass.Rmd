...
```{r}
ggplot(ncaa) +
  geom_step(aes(year, total)) +
  geom_step(aes(year, baseline)) +
  geom_rect(aes(
    xmin = year, 
    xmax = year + 1, 
    ymin = baseline, 
...
You could use geom_ribbon to fill the area in between, but it's hard to fill the area in
between with a different color according to the difference. Instead, you could use
geom_segment and 
