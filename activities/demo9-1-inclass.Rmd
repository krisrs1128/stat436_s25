...
library(tidymodels)
centroids <- tidy(fit) |>
  pivot_longer(starts_with("19"), names_to = "date") |>
...
x_clust |> 
  ...
  mutate(date = as_date(date)) |>
  ggplot() +
    geom_tile(aes(date, reorder(state, value), col = log(1 + value), fill = log(1 + value)), width = 7) +
    facet_grid(reorder(.cluster, -value) ~ ., scales = "free_y", space = "free_y") +
